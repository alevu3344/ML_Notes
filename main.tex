...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{plots/qda_plot.png}
	\caption{A QDA classifier on synthetic 2D data. The decision boundaries (solid lines) are quadratic curves. The dotted ellipses show the Gaussian contours (one standard deviation) for each class, illustrating how QDA adapts to the different covariance structures (shapes and orientations) of the data.}
	\label{fig:qda_plot}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{plots/generative_classifier_comparison.png}
	\caption{A comparison of QDA, LDA, and NMC trained on the same dataset.
		\textbf{(Left)} QDA's quadratic boundaries adapt to the unique covariance of each class.
		\textbf{(Center)} LDA's linear boundaries result from assuming a single pooled covariance for all classes.
		\textbf{(Right)} NMC's boundaries (marked with 'X' at the class means) are also linear but are further restricted, forming perpendicular bisectors between the class means, which results from assuming a spherical covariance ($\boldsymbol{\Sigma} = \sigma^2\mathbf{I}$).}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\textwidth]{plots/density_estimation_plot.png}
	\caption{A comparison of non-parametric density estimation techniques on a bimodal dataset.
		\textbf{(Top)} A histogram with a large bin width ($h=2.5$) is overly smooth and fails to capture the bimodal structure (high bias).
		\textbf{(Middle)} A histogram with a small bin width ($h=0.15$) is too noisy and fits to random fluctuations in the data (high variance).
		\textbf{(Bottom)} Kernel Density Estimation (KDE) with a well-chosen bandwidth ($h=0.5$) provides a smooth and accurate representation of the underlying density.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{plots/knn_boundaries_plot.png}
	\caption{The k-NN decision boundary's dependence on the hyperparameter $k$.
		\textbf{(Left)} With $k=1$, the boundary is highly complex and "noisy," fitting to individual data points (low bias, high variance).
		\textbf{(Right)} With a larger $k=15$, the boundary becomes much smoother by averaging over a larger neighborhood, leading to a simpler model (high bias, low variance).}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{plots/dimensionality_plot.png}
	\caption{A visual illustration of the curse of dimensionality. This plot shows the ratio of the volume of a hypersphere (with radius $r=1$) to the volume of the hypercube that encloses it (with side length $L=2$). As the number of dimensions ($d$) increases, the volume of the hypersphere plummets towards zero relative to the cube's volume. This shows that in high dimensions, almost all of the hypercube's volume is concentrated in its "corners," outside the inscribed sphere.}
	\label{fig:dimensionality_plot}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{plots/perceptron_update.png}
	\caption{Geometric intuition of the Perceptron update. A positive sample $\mathbf{x}_i$ (blue) is misclassified by the boundary defined by $\mathbf{w}_{\text{old}}$ (blue). The update rule $\mathbf{w}_{\text{new}} = \mathbf{w}_{\text{old}} + \mathbf{x}_i$ (with $\eta=1$) rotates the weight vector and its perpendicular decision boundary to a new position (red) that correctly classifies $\mathbf{x}_i$.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{plots/fld_projection.png}
	\caption{An illustration of the Fisher Linear Discriminant.
		\textbf{(Top)} A naive projection onto the $x_1$ axis results in poor separation, as the projected 1D histograms have significant overlap. This projection maximizes within-class scatter.
		\textbf{(Bottom)} The optimal projection found by FLD (red line) aims for a direction that maximizes the distance between the projected class means (blue and red dots) while minimizing their spread. The resulting 1D histograms are well-separated.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{plots/least_squares_outlier.png}
	\caption{The sensitivity of the Least Squares classifier to outliers.
		\textbf{(Left)} On a clean, separable dataset, both the Least Squares (blue) and Logistic Regression (red) find a good decision boundary.
		\textbf{(Right)} After adding a single, distant outlier (the blue 'X'), the Least Squares model is heavily skewed. It rotates its boundary (blue) to reduce the large squared error from this point, leading to a poor classification. Logistic Regression (red) is robust to this outlier, as it only cares about the boundary region.}
	\label{fig:least_squares_outlier}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{plots/sigmoid_function.png}
	\caption{The logistic (sigmoid) function. It takes the output of the linear model $z = \mathbf{w}^T\mathbf{x} + w_0$ (which can be any real number) and "squashes" it into a valid probability between 0 and 1. The decision boundary $z=0$ corresponds to a probability of $0.5$.}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth]{plots/bias_variance_tradeoff.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{plots/feature_map_plot.png}
